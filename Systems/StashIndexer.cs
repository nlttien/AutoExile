...
namespace AutoExile.Systems
{
    ...
    public class StashIndexer
    {
        ...
        private void TickReadTab(GameController gc, ExileCore.PoEMemory.Elements.StashElement stashEl, IList<string> names)
        {
            ...
            if (items != null)
            {
                foreach (var slot in items)
                {
                    ...
                    // Map tier extraction isolated in a helper so TypeLoadException on the
                    // Map component (seen on some ExileCore builds) doesn't break the whole scan.
                    int mapTier = TryGetMapTier(entity);
        ...
        /// <summary>
        /// Extract map tier via reflection so a missing <c>ExileCore.PoEMemory.Components.Map</c>
        /// type (some ExileCore builds) can't trigger a JIT-time TypeLoadException when this
        /// method is compiled. Returns 0 if the type or tier isn't available.
        /// </summary>
        private static int TryGetMapTier(ExileCore.PoEMemory.MemoryObjects.Entity entity)
        {
            try
            {
                var mapType = Type.GetType("ExileCore.PoEMemory.Components.Map, ExileCore", throwOnError: false);
                if (mapType == null) return 0;
                var getComponent = typeof(ExileCore.PoEMemory.MemoryObjects.Entity)
                    .GetMethod("GetComponent", System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)
                    ?.MakeGenericMethod(mapType);
                var component = getComponent?.Invoke(entity, null);
                if (component == null) return 0;
                var tierVal = mapType.GetProperty("Tier")?.GetValue(component);
                return tierVal switch { byte b => b, int i => i, _ => 0 };
            }
            catch { return 0; }
        }
